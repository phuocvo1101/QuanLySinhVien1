 ...
 public function listPages($totalPages){
  ...
  if($totalPages > 1){ // sá»‘ trang pháº£i tá»« 2 trang trá»Ÿ lÃªn
   ...
   if($current != 1){ // NÃºt prev
    ...
    $listPage .= "<a onclick='getBooks(".$totalPages.",".$newstart.",".$limit.");' href='javascript:void(0);'>Prev</a>";
   }
   ...
   for($i=1;$i<=$totalPages;$i++){  // Táº¥t cáº£ cÃ¡c trang tÃ¬m Ä‘Æ°á»£c
    ...
    if($i == $current){
     ...
     $listPage .= "<a onclick='getBooks(".$totalPages.",".$newstart.",".$limit.");' href='javascript:void(0);'>".$i."</a>";
    }
   }
    
   if($current != $totalPages){ // NÃºt next
    $newstart = $start + $limit;
    $listPage .= "<a onclick='getBooks(".$totalPages.",".$newstart.",".$limit.");' href='javascript:void(0);'>Next</a>";
   }
  }
